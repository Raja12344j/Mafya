...
def load_user_data():
    ...
    try:
        if os.path.exists(USER_DATA_FILE):
            with open(USER_DATA_FILE, 'r') as jf:
                d = json.load(jf)
                user_names.update(d.get('user_names', {}))
                # restore approved and pending if present
                for uid in d.get('approved', []):
                    approved_users.add(uid)
                for uid in d.get('pending', []):
...
def save_user_data():
    try:
        with open(USER_DATA_FILE, 'w') as jf:
            json.dump({'user_names': user_names, 'approved': list(approved_users), 'pending': list(pending_requests)}, jf)
    except Exception as e:
...
def admin_panel():
    if not session.get('admin_logged_in'):
        return redirect(url_for('admin_login'))
    # build lists with display names
    pending = []
    for uid in pending_requests:
        pending.append({'id': uid, 'name': user_names.get(uid, '')})
    approved = []
    for uid in approved_users:
        approved.append({'id': uid, 'name': user_names.get(uid, '')})
